#!/usr/bin/env python3
"""Validate a cleaned SRT against its raw evidence track.

The checker is intentionally conservative. It is not a replacement for
listening to the audio; it catches structural risks that commonly appear when
an agent cleans ASR subtitles for downstream note generation.
"""
...
def parse_srt(path: Path) -> list[Entry]:
    ...
    for block in blocks:
        ...
        start_ms, end_ms = parse_time(lines[1])
        entries.append(
            Entry(
...
def raw_id_from_clean_id(block_id: str) -> int | None:
    match = re.match(r"^(\d+)(?:[A-Za-z]+)?$", block_id.strip())
    if not match:
        return None
...
def find_neighbor_shift_signal(raw_entries: list[Entry], index: int, clean_text: str) -> str | None:
    current_ratio = text_similarity(raw_entries[index].text, clean_text)
    ...
    for label, _text, neighbor_ratio in candidates:
        if neighbor_ratio >= 0.72 and neighbor_ratio >= current_ratio + 0.20:
            return f"possible text carryover from {label} raw subtitle (self={current_ratio:.2f}, {label}={neighbor_ratio:.2f})"
    return None


def main() -> int:
    parser = argparse.ArgumentParser(description="Check clean.srt against raw.srt for timing and text constraints.")
    parser.add_argument("raw_srt", type=Path, help="Path to raw.srt")
    parser.add_argument("clean_srt", type=Path, help="Path to clean.srt")
    parser.add_argument("--allowed-deletions", default="", help="Comma-separated raw ids allowed to be deleted")
    parser.add_argument("--min-duration-ms", type=int, default=400, help="Warn when a cleaned subtitle is shorter")
    parser.add_argument("--max-visible-length", type=int, default=24, help="Maximum visible chars per cleaned subtitle")
    parser.add_argument("--fail-on-warnings", action="store_true", help="Return non-zero when warnings are found")
    parser.add_argument("--latin-word-as-one-char", action="store_true", help="Count Latin alnum runs as one char")
    ...
    raw_by_id = {raw_id_from_clean_id(entry.block_id): entry for entry in raw_entries}
    issues: list[str] = []
    warnings: list[str] = []
    grouped: dict[int, list[Entry]] = {
        int(entry.block_id): [] for entry in raw_entries if raw_id_from_clean_id(entry.block_id) is not None
    }

    if len(raw_by_id) != len(raw_entries):
        warnings.append("raw subtitles contain non-numeric ids; clean split ownership checks may be incomplete")

    for entry in clean_entries:
        owner_id = raw_id_from_clean_id(entry.block_id)
        if owner_id is None:
            issues.append(f"{entry.block_id}: id must be numeric or split suffix like 123a")
            continue
        ...
        if visible_length(text, args.latin_word_as_one_char) > args.max_visible_length:
            issues.append(f"{entry.block_id}: text longer than {args.max_visible_length} visible chars in {text!r}")
        ...
        if entry.end_ms - entry.start_ms < args.min_duration_ms:
            warnings.append(f"{entry.block_id}: short duration {entry.end_ms - entry.start_ms}ms in {text!r}")
        ...
        owner = raw_by_id.get(owner_id)
        if owner is None:
            issues.append(f"{entry.block_id}: no matching raw subtitle id {owner_id}")
            continue
        if entry.start_ms < owner.start_ms or entry.end_ms > owner.end_ms:
            issues.append(f"{entry.block_id}: outside original range")
            continue
        grouped.setdefault(owner_id, []).append(entry)

    raw_index_by_id = {
        raw_id_from_clean_id(entry.block_id): index
        for index, entry in enumerate(raw_entries)
        if raw_id_from_clean_id(entry.block_id) is not None
    }

    for raw_id, original in raw_by_id.items():
        if raw_id is None:
            continue
        parts = sorted(grouped.get(raw_id, []), key=lambda item: (item.start_ms, item.end_ms, item.block_id))
        if not parts:
            raw_text = compact_text(original.text)
        ...
        if len(parts) == 1:
            ...
            if only_part.start_ms != original.start_ms or only_part.end_ms != original.end_ms:
                ...
                index = raw_index_by_id.get(raw_id)
